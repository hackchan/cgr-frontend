...
export const Register = ({ setModal, setReload, preData, AddFormaContrato, modedark }) => {
  ...
  const onSubmit = async (dataForm) => {
    try {
      setDisableBtn(true)
      await AddFormaContrato(dataForm)
      setModal(false)
      setReload(true)
    } catch (error) {
      if (error.response) {
      ...
      <Form onSubmit={handleSubmit(onSubmit)}>
        ...
        <Row className='mb-3'>
          <Form.Group as={Col} controlId='name'>
            ...
            <Form.Control style={{ height: 38 }} type='text' placeholder='Eje: DIRECTA' {...register('name', { required: `nombre del ${preData.table} obligatorio` })} />
