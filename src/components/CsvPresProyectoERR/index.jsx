...
import { formSchemaProyectoArray } from '../CrudMatrizProyecto/Schema'
import { ColumnsTable } from './Columns'
import config from '../../config'
// import { ButtonLoading } from '../ButtonLoading'
// import { object, string, number, array, date } from 'yup'

export const CsvParserPresProyectoERR = ({ data, setModalCsv, setReload, MatrizCargada, onCloseFile }) => {
  ...
  const handleValidateData = async () => {
    try {
      ...
      setProgress(true)
      setErrorDetail([])
      formSchemaProyectoArray.validateSync(tableData, { abortEarly: false })
      setUpload(false)
    } catch (error) {
      setUpload(true)
      setBtnLoadingBlock(true)
  ...
  const handleSaveCell = (cell, value) => {
    ...
    if (cell.column.id === 'entidad' || cell.column.id === 'sector' || cell.column.id === 'duracionProyecto') {
      if (!isNaN(parseInt(value))) {
        value = parseInt(value)
      }
    } else if (cell.column.id === 'valorProyecto') {
